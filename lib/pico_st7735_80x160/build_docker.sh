...
#
# The project to build is taken from the current working directory, so run it
# from inside the project folder you want to build, e.g.:
#   cd test
...
PROJECT_ROOT="$(cd "$(dirname "$0")" && pwd)"    # repository root
...
# Project is determined by the caller's current working directory.
PROJECT_DIR="$PWD"
PROJECT_NAME="$(basename "$PROJECT_DIR")"

usage() {
  cat <<USAGE
Usage: (cd <project> && ../$(basename "$0") [target] [options])

The project to build is taken from the current working directory. Run this
script from inside the project folder you want to build (e.g. test).
...
# Must be run from a project directory directly under the repository root that has a CMakeLists.txt.
if [[ "$(dirname "$PROJECT_DIR")" != "$PROJECT_ROOT" || ! -f "$PROJECT_DIR/CMakeLists.txt" ]]; then
  echo "error: run this from inside a project folder under the repository root (current: $PROJECT_DIR)" >&2
  usage
  exit 1
fi

# Path of the project relative to the repository root (mounted at /workspace).
PROJECT_REL="$PROJECT_NAME"
...
run_build() {
  ...
  docker run --rm \
    ...
    -w "/workspace/$PROJECT_REL/$build_dir" \
    "$IMAGE" \
    bash -c "cmake $cmake_extra /workspace/$PROJECT_REL && make -j\$(nproc)"
}
...
for d in "${BUILT_DIRS[@]}"; do
  for uf2 in "$PROJECT_DIR/$d"/*.uf2; do
    [[ -f "$uf2" ]] && echo "  $PROJECT_REL/$d/$(basename "$uf2")"
  done
done
