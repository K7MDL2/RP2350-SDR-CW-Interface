...
static u8 fill_buffer[LCD_WIDTH * 2];
...
{
    uint8_t data[2] = {(uint8_t)(dat >> 8), (uint8_t)dat};

    OLED_DC_Set();//write data
    OLED_CS_Clr();
    spi_write_blocking(_config.spi_inst, data, sizeof(data));
    OLED_CS_Set();
}
...
void LCD_Fill(u16 xsta,u16 ysta,u16 xend,u16 yend,u16 color)
{
    ...
    if (width <= LCD_WIDTH)
    {
        ...
        {
            fill_buffer[column * 2u] = (u8)(color >> 8);
            fill_buffer[column * 2u + 1u] = (u8)color;
        }
        ...
        {
            spi_write_blocking(_config.spi_inst, fill_buffer, width * 2u);
        }
        OLED_CS_Set();
    }
    else
    {
        for (row = ysta; row <= yend; ++row)
        {
            for (u16 column = xsta; column <= xend; ++column)
            {
                LCD_WR_DATA(color);
            }
        }
